"""
Optimizer to study placement and partial ordering of tasks in a single/multi device scenario
for a set of tasks.
Precedence is defined between tasks explicitly.
...
def setup_graph_info(args):
    ...
    for node_idx in graph_utils.G.nodes():
        ...
        order_info[node_idx]  = graph_utils.get_node_ancestors(node_idx)
...
def get_dummy_info():
    ...
    device_list = ['cpu', 'gpu']
    task_list = [0, 1, 2, 3, 4]
    service_times = {'cpu' : {0:3, 1:3, 2:1, 3:2, 4:3}, 'gpu': {0:2, 1:3, 2:2, 3:2, 4:3}}
    # Map of node to its predecessors and the data transfer incurred for that predecessor
    #pred_info = {0: [], 1: [(0,1)], 2: [(1,1)], 3: [], 4: []}
    pred_info = {0: [(1, 0)], 1: [(2, 0)], 2: [(3, 0)], 3: [], 4: []}
    order_info = {0: [1], 1: [2], 2: [3], 3: [], 4: []}
    PCIE_BW = 1.0
    return task_list, device_list, service_times, pred_info, order_info
...
def optimize_placement_and_ordering(args, task_list, device_list, service_times, pred_info, order_info):
    ...
    for i in task_list:
        ...
        for j in task_list:
            if i == j:
                ordering[i][j] = solver.IntVar(0.0, 0.0, 'P_' + str(i) + '_' + str(j))
            else:
                ordering[i][j] = solver.IntVar(0.0, 1.0, 'P_' + str(i) + '_' + str(j))
    ...
    for i in task_list:
        for j in task_list:
            if i == j:
                continue
            # P_i_j + P_j_i = 1.0
            constr3 = solver.Constraint(1.0, 1.0)
            constr3.SetCoefficient(ordering[i][j], 1)
            constr3.SetCoefficient(ordering[j][i], 1)
    ...
    # If not executing on a particular device, set FinishTime = 0, else calculate it
    # FinishTime_d_i - StartTime_d_i + (D_d_i * H) >= 0, -FinishTime_d_i + StartTime_d_i + (D_d_i * H) >=0
    #for d in device_list:
    #    for i in task_list:
    #        constr1a = solver.Constraint(0.0, inf)
    #        constr1a.SetCoefficient(finish_times[d][i], 1)
    #        constr1a.SetCoefficient(start_times[d][i], -1)
    #        constr1a.SetCoefficient(placement[d][i], LARGE_NUM)
    #        constr1b = solver.Constraint(0.0, inf)
    #        constr1b.SetCoefficient(finish_times[d][i], -1)
    #        constr1b.SetCoefficient(start_times[d][i], 1)
    #        constr1b.SetCoefficient(placement[d][i], LARGE_NUM)
    ...
    for (pred, succ) in precedence_list:
        # ordering[pred][succ] = 1.0
        fwd_pred_constr = solver.Constraint(1.0, 1.0)
        fwd_pred_constr.SetCoefficient(ordering[pred][succ], 1)
        
        # ordering[succ][pred] = 0.0
        rev_pred_constr = solver.Constraint(0.0, 0.0)
        rev_pred_constr.SetCoefficient(ordering[succ][pred], 1)
    ...
    # Partial order list
    partial_precedence_list = []
    for (succ, ancestors) in order_info.items():
        for pred in ancestors:
            # ordering[pred][succ] = 1.0
            fwd_pred_constr = solver.Constraint(1.0, 1.0)
            fwd_pred_constr.SetCoefficient(ordering[pred][succ], 1)
    ...
    def write_solution(solver, out_file):
        ...
        for i in task_list:
            for j in task_list:
                out_file.write(str(ordering[i][j].solution_value()) + ", ")
            out_file.write("\n")
